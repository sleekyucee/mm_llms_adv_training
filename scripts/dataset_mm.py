...
def _train_tf_clip(image_size: int):
    aug = [
        transforms.RandomResizedCrop(image_size, scale=(0.9, 1.0), ratio=(0.98, 1.02),
                                   interpolation=InterpolationMode.BICUBIC),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomApply([
            transforms.ColorJitter(brightness=0.2, contrast=0.15, saturation=0.10, hue=0.02)
        ], p=0.6),
        transforms.RandomApply([
...
def _eval_tf_clip(image_size: int):
...
class FFPPTripletDataset(Dataset):
    def __init__(
        ...
        if clip_norm:
            self.tf = _train_tf_clip(image_size) if augment else _eval_tf_clip(image_size)
            ...
            self.tf2 = transforms.Compose(self.tf.transforms) if self.return_pair else None
    ...
    def _load_img_pil(self, p: Path) -> Image.Image:
